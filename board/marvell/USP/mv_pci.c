...
unsigned char g_pciDeviceStatus=0;//for manufacture test
...
MV_BOOL scanPci(MV_U32 host)
{
    ...
    {
		if(pciDevices[index].busNumber==0 && pciDevices[index].deviceNum==1 && 
			pciDevices[index].venID==0x11ab && pciDevices[index].deviceID==0x8120)
			g_pciDeviceStatus=1;//found pci device

		if(pciDevices[index].busNumber==1 && pciDevices[index].deviceNum==1 && 
			pciDevices[index].venID==0x11ab && pciDevices[index].deviceID==0x8120)
			g_pciDeviceStatus=1;//found pci device
...
pci_init_board(void)
{
	...
	char *env;
	int first_busno=0;
	int status; unsigned long uiReg;
	...
	for (pciIf = 0; pciIf < pciIfNum; pciIf++)
	{
		...
		if (PCI_IF_TYPE_PEX == mvPciIfTypeGet(pciIf))
		{
            ...
            !defined(MV88F6192) && !defined(MV88F6180) && !defined(MV88F6781Z0) && !defined(MV88F6781Y0) && \
            !defined(MV88F6781X0)
			MV_PEX_MODE pexMode;
...
// 			}
		...
		if ((status = mvSysPciIfInit(pciIf, pciIfMode)) == MV_ERROR)
		{
