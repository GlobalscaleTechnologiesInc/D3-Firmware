...
/*
	...
	printf("******************************\n");
*/
//	printf("Pls wait for few seconds...***\n");
/*
...
/*	
		do_nand(cmdtp,flag,2,argv);
		argv[1]="bad"; //for testing nand flash bad blocks
		do_nand(cmdtp,flag,2,argv);
		printf("******************************\n");
		if(g_nand_bad_block_count<MAN_NAND_MAX_BAD_BLOCKS)
		{
			sprintf(nand_addr,"%llx\0",g_nand_good_block_addr);
			argv[1]="read"; //backup nand flash data to prevent from erasing nand data 
			argv[2]="0x2000000"; 
			argv[3]=nand_addr;	
			argv[4]="0x200000";//one block size	
			do_nand(cmdtp,flag,5,argv);

			//start testing nand flash
			argv[1]="erase";  //start testing nand flash
			argv[2]=nand_addr; 
			argv[3]="0x200000";//one block size	
			do_nand(cmdtp,flag,4,argv);
			...
			*paddr=0x0;//clear mem to check if nand read ok
			argv[1]="read"; 
			argv[2]="0x3000000"; 
			argv[3]=nand_addr;	
			argv[4]="0x200000";//one block size	
			do_nand(cmdtp,flag,5,argv);
			//end testing nand flash

			argv[1]="erase";  //restore nand flash data from dram
			argv[2]=nand_addr; 
			argv[3]="0x200000";//one block size	
			do_nand(cmdtp,flag,4,argv);
			argv[1]="write"; 
			...
			{
				printf("Read/Write NAND Flash Failed!!\n");
				return 1;	
			}	
		...
		{
			printf("NAND Flash has %d bad blocks over %d!!\n",g_nand_bad_block_count,MAN_NAND_MAX_BAD_BLOCKS);
			return 1;	
		}
*/		
/*		
