...
-G exec_file   ASCII file name that contains binary routine (raw ARM 5TE code
      without ELF header) to run before the boot loader image execution.
      The routine must contain array of parameters placed before the 
      entry point and an appropriate ARM code for saving all registers 
      to stack at procedure start and restore them before return from
      the routine. The code should be linked to a right RAM address 
      and take into account the amount of input parameters.
      See the example code and the linker script in "binhdr" directory.

-K args_num     The number of arguments to be passed to the binary routine. 
      This parameter is used by the SOC firmware for the binary 
      header routine entry point calculation.
      Maximum number of arguments is 255. 
      If omitted, assigned default value of 0.
...
-P nand_pg_sz    Decimal number configuring the nand page size. 
      Typical values are 512 and 2048.

-C nand_ecc     Nand ECC algorithm (1=Hamming, 2=RS, 3=None)
